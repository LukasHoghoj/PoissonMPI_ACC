...
void poisson3D_hyb5(double *A, double *Anew, double *S, int nx, int ny, int nz, double dx, double dy, double dz, int Niter, int rank, int size){
    double dxs, dys, dzs;
    ...
    if(rank==0){
    printf("WARNING: This method is not working correctly yet, results will be inaccurate\n");
    }
    ...
    for(int N = 0; N < Niter; N++){
        ...
        #pragma acc parallel loop present(A[0:nx*ny*nz], S[0:nx*ny*nz],Anew[0:nx*ny*nz],dsum,third, sixth) reduction(+:Fnorm) tile(32,32) async(i)
        ...
        #pragma acc wait(1)
        #pragma acc wait(nz-2)


        #pragma acc data present(Anew)
        {
        #pragma acc host_data use_device(Anew)// data deviceptr(Anew) //
        {
        if(rank > 0){
