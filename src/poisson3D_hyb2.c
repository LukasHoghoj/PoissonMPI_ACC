...
void poisson3D_hyb2(double *A, double *Anew, double *S, int nx, int ny, int nz, double dx, double dy, double dz, int Niter, int rank, int size){
    double dxs, dys, dzs;
    dxs = dx*dx; dys = dy*dy; dzs = dz*dz;
    double dsum = dxs + dys + dzs;
    double *tmp;
    double Fnorm, Fnormold=0, Fdiff;
    #pragma acc data copyin(A[0:nx*ny*nz], Anew[0:nx*ny*nz], S[0:nx*ny*nz])
    {
	...
	// Change to only the required data
        // Communicate BC
        #pragma acc update host(A[0:nx*ny*nz])
        if(rank > 0){
        ...
        #pragma acc update device(A[0:nx*ny*nz])
    }
}
}
